		...
		function index(){ 
			...
			*/	
			...
			/*
				...
				|----------------------------------------------------------------------------------------------------
			*/
			if (isset($_GET['recherche_id']) && $_GET['rechercheid'] != NULL ) {
				$donnees=$this->Model_fiche->fiche($_GET['rechercheid']);
				}elseif(isset($_GET['rechercher_id']) && $_GET['rechercheid'] == NULL){
				header("Location: ".base_url().'/administration');
			}
			/*
				|----------------------------------------------------------------------------------------------------
				Fonction pour équivalences
				|----------------------------------------------------------------------------------------------------
			*/
			...
			*/	
			if (isset($_GET['rechercher_patronyme']) && $_GET['patronyme'] != NULL ) {
				...
				// $requete_patro = 'a.patronyme REGEXP "('.$mots[0].')" OR a.famille REGEXP "('.$mots[0].')" ' ; 
				$requete_patro = 'a.patronyme REGEXP "('.$_GET['patronyme'].')" OR a.famille REGEXP "('.$_GET['patronyme'].')" ' ; 
			...
			*/
			if(isset($_GET['rechercher_ordre']) || isset($_GET['rechercher_desordre'])){
				...
				*/
				/*
					|----------------------------------------------------------------------------------------------------
					| OLD Champ PATRONYME (ET)
					|----------------------------------------------------------------------------------------------------
				*/
				// if ($_GET['mot_2'] != NULL ) {
				// 	// $mots = explode(' ',$_GET['mot_2'])	; // Récupération des mots-clé
				// 	// ET
				// 	//$requete_patro = 'a.patronyme REGEXP "('.$_GET['mot_2'].')" OR a.famille REGEXP "('.$_GET['mot_2'].')"' ; 
				// 	$requete_patro = 'a.patronyme REGEXP "('.$_GET['mot_2'].')" OR a.famille REGEXP "('.$_GET['mot_2'].')"' ; 
				...
				*/
				...
				if ($_GET['mot_2'] != NULL ) {
					...
					$requete_patro = 'a.patronyme like "%'.$string.'%" OR a.famille like "%'.$string.'%"';
				...
				*/
				...
				/*
					|----------------------------------------------------------------------------------------------------
					| OLD Champ REF BIBLIO
					|----------------------------------------------------------------------------------------------------
				*/
				// if ($_GET['ref_biblio'] != NULL ) {
				// 	$mots = str_replace(array('(', ')'), ' ', $_GET['ref_biblio']);
				// 	$mots = explode(' ', $mots); // Récupération des mots-clé
				...
				*/
				if ($_GET['ref_biblio'] != NULL ) {
					...
					$string = $_GET['ref_biblio'];
					//ET
					
					$requete_ref = 'a.ref_biblio = "'.$string.'"';
			...
			*/
			...
			if ($publique == true && $session == false) {
