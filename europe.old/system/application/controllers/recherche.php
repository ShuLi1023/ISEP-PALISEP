<?php
	class recherche extends Controller {  // Le nom de la classe est le nom du contrôleur avec une majuscule
		function index(){ 
			...
			*/
			if (isset($_POST['rechercher_patronyme']) && $_POST['patronyme'] != NULL) {	
				...
				$requete_patro = 'a.patronyme REGEXP "('.$_POST['patronyme'].')" OR a.famille REGEXP "('.$_POST['patronyme'].')" ' ; 
				// for($i=1; $i<count($mots) ; $i++){
				// $requete_patro = $requete_patro.'AND a.patronyme REGEXP "('.$mots[$i].')" OR a.famille REGEXP "('.$mots[$i].')" ' ;
				// }
			...
			*/
			if (isset($_POST['rechercher_region']) && $_POST['region'] != NULL) {	
				$mots = explode(' ',$_POST['region'])	; // Récupération des mots clés
			...
			*/
			if (isset($_POST['rechercher_fief']) && $_POST['fief'] != NULL) {	
				$mots = explode(' ',$_POST['fief'])	; // Récupération des mots clés	
			...
			*/		
			if (isset($_POST['rechercher_departement']) && $_POST['departement'] != NULL) {	
				$mots = explode(' ',$_POST['departement'])	; // récupération des mots clés
			...
			*/
			/*
				|----------------------------------------------------------------------------------------------------
				| Recherche par ARMES (formulaire croisé avec les champs : armes, fief, patronyme, siècle, département, région dénomination)
				|----------------------------------------------------------------------------------------------------
			*/
			if(isset($_POST['rechercher_ordre']) || isset($_POST['rechercher_desordre'])){
				...
				*/
				if (isset($_POST['rechercher_desordre']) && $_POST['mot'] != NULL) {  // ET mots dans le désordre
					...
					$mots = explode(' ',$_POST['mot'])	; // Récupération des mots clés
				...
				*/
				if ($_POST['mot_2'] != NULL) {
					// $mots = explode(' ',$_POST['mot_2'])	; // Récupération des mots clés
					// ET
					$requete_patro = 'a.patronyme REGEXP "('.$_POST['mot_2'].')" OR a.famille REGEXP "('.$_POST['mot_2'].')"' ; 
					// for($i=1; $i<count($mots) ; $i++){
					// $requete_patro = $requete_patro.'AND a.patronyme REGEXP "('.$mots[$i].')" OR a.famille REGEXP "('.$mots[$i].')"' ;
					// }
				...
				*/					
				if ($_POST['mot_3'] != NULL) {
					...
					$mots = explode('+',$_POST['mot_3'])	; // Récupération des mots-clé	
					// OU 
				...
				*/
				...
				if ($_POST['mot_4'] != NULL) {
					...
					$mots = explode('+',$_POST['mot_4'])	; // Récupération des mots-clé	
					// OU
				...
				*/
				if ($_POST['mot_5'] != NULL) {
					...
					$mots = explode('+',$_POST['mot_5'])	; // Récupération des mots clés	
					// OU 
				...
				*/		
				if ($_POST['Denomination'] != NULL) {
					...
					$req1[] = '(l.dénomination = "'.$_POST['Denomination'].'"AND l.vedette_chandon = a.patronyme)';
					$req2[] = '(l.dénomination = "'.$_POST['Denomination'].'"AND l.vedette_chandon = a.patronyme)';
					// $donnees=$this->Model_legende_photos->infos_Denomination($requete_Denomination);
				}
				// $requete_sceau = "";
				// if(!isset($_POST['sceau']) && $_POST['sceau'] != null) {	
				// $requete_sceau = 'l.dénomination LIKE "sceau"';
				// $req1[] = $requete_sceau;
				// }
				...
				*/
				if ($_POST['mot_6'] != NULL) {
					$mots = explode('+',$_POST['mot_6'])	; // Récupération des mots clés
					// OU 
				...
				*/
				/*
					|----------------------------------------------------------------------------------------------------
					| Validation formulaire croisé + envoi de la requête SQL
					|----------------------------------------------------------------------------------------------------
				*/
