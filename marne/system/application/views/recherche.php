...
if(isset($_POST['rechercher_desordre']) || isset($_POST['rechercher_ordre'])){ ?>
		<h4><a href="javascript:history.go("http://www.marne.palisep.fr/recherche")"><<< Retour sur le formulaire</a></h4>
...
<center><table >	
	<tr rules="rows">
		...
		<?php	foreach($donnees1 as $data1) { 
			...
			?>
		<tr>
		<!-- patronyme--><td ><li><strong><?php echo anchor('fiche/armorial/'.$data1->id, ucfirst(mb_strtolower($data1->patronyme,'UTF-8')));?></strong>, <!-- fief--><em><?php if (!empty($data1->fief)){ echo $data1->fief; }?></em></li>
		...
		<?php foreach($photos as $photo){ 
			if ($photo->vedette_chandon == $data1->patronyme){ ?>
				...
				<a href="<?php echo base_url() ?>resources/images/<?php echo $photo->libellé_image; ?>.jpg" rel="lightbox[roadtrip]">
					<img height="100px" src="<?php echo base_url()  ?>resources/images/<?php echo $photo->libellé_image; ?>.jpg"/>
				</a>
			<?php } 
		} ?> 
		</td>
...
<center><table border="1" bordercolor="#C9C9C9" frame="hsides" rules="rows">
	<?php foreach($donnees as $data) { ?>
		<tr>
		<!-- patronyme--><td width="40%"><li><strong><?php echo anchor('fiche/armorial/'.$data->id, ucfirst (mb_strtolower($data->patronyme,'UTF-8')));?></strong></li>
...
<!-- COLONNE DROITE-->
	...
	<form action="recherche" method="post">
	<label class="label1" style='width:200px;'><strong>Titres, fiefs et dignités</strong></label>
...
<fieldset>
	...
	<form action="recherche" method="post">	
		<center>
			<label class="label1"><strong>Armes (<a href='javascript:getDico();'>dictionnaire</a>)</strong></label>
			<input id="mot" type="text" name="mot" size="60%" <?php if (isset($_POST['mot'])){ ?>value="<?php echo $_POST['mot'] ;?>"<?php }; ?>/><br /><br />
	...
	<div id="gauche_armes">
		...
		<label class="label2">Titres, fiefs et dignités</label>
...
<div id="dico" title="Dictionnaire imagé"></div> <!--Div pour l'affichage du dico -->
